...
Before using the script, you should know how to config files to run the script. One is a yaml format file that config
...
In short, you should config a yaml format file when using all functions(build, edit, load), and a json format file when
...
```shell
omni-imager build anaconda-iso --config-file /opt/omni-imager/etc/conf.yaml \
...
- omni-imager build ${image-type} ...: The type of ISO you want to build, currently supported ISO with anaconda installer
...
```shell
omni-imager build calamares-iso --package-list /etc/omni-imager/openEuler-minimal.json --config-file /etc/omni-imager/conf.yaml \
...
```shell
omni-imager load ks --config-file /etc/omni-imager/conf.yaml
...
```shell
omni-imager edit ks --ks ${ks.cfg file path}  --config-file /etc/omni-imager/conf.yaml
